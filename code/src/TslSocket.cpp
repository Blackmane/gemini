...
gemini::TslSocket::TslSocket(const std::string hostname,
...
void gemini::TslSocket::connect(const std::string hostname,
                                const std::string port) {
  ...
  if (status != 1) {
    ...
    throw std::runtime_error("SSL_connect failed with SSL_get_error code " +
                             status);
  }
...
void gemini::TslSocket::printCerts() {
  std::cout << "Connected with " << SSL_get_cipher(_ssl) << " encryption"
            << std::endl;
  displayCerts(_ssl);
}